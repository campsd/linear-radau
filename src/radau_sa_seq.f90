...
! radau_sa_seq
! ------------------------------------------------------------------------------------------------------
! Sequential implementation of IRK integrator radauIIA with semi-adaptive stepsize
! ------------------------------------------------------------------------------------------------------
! Nov. 2016
! ------------------------------------------------------------------------------------------------------
...
module radau_sa_mod
	...
	contains
		...
		subroutine radau_sa( A, RI, CJ, N, NNZ, YZERO, TEND, HINIT, HEVALS, YFINAL)
		...
		! I/O
		integer, intent(in)	::	N, NNZ, HEVALS	
		...
		! subroutine specific
		integer 			::	IERR, NNZNR, NNZNC, NSTEPS, i, j, k, l, world_id, world_size
		real(wp), allocatable		::	ANR(:)
		complex(wp), allocatable	::	ANC(:)
		integer, allocatable		::	RINR(:), CJNR(:), RINC(:), CJNC(:)	
		real(wp)			::	YCURR(N), YPREV(N), R1(N), R2(N), R3(N), W1(N), W2(N), TCURR, HCURR, HPREV, AERRPREV, EVALPOINTS(HEVALS+2)
                integer p, id, cnt
		logical	CHANGE
		...
		! DETERMINE STEPSIZE EVALUATION POINTS
		call logspace(1.0D0,TEND,HEVALS+2,EVALPOINTS)
		write(*,*) EVALPOINTS
		...
		do !while (TCURR < TEND)
			...
			elseif ((TCURR .lt. EVALPOINTS(l)) .and. (TCURR + HCURR .ge. EVALPOINTS(l)) .and. (EVALPOINTS(l) .lt. TEND)) then
				write(*,*) 'STEP EVALUATION #', l-1
		...
		end subroutine radau_sa

		subroutine logspace(xmin, xmax, n, ls)
			real(wp), intent(in)	::	xmin, xmax
			integer, intent(in)	::	n
			real(wp), intent(out)	::	ls(n)
			integer i
			do i = 1,n
				ls(i) = 10 ** ((log10(xmax) - log10(xmin)) * real(i-1,wp) / real(n-1,wp) + log10(xmin))
			enddo
